...
class VatValidationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = VatValidationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = VatValidationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, VatValidationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    VatValidationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = VatValidationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = VatValidationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = VatValidationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = VatValidationError.new(
        "graphql_error", "VatValidationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = VatValidationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
