...
module VatValidationUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = VatValidationContext.new({}, nil)
    end
    op = ctx.op || VatValidationOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || VatValidationResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(VatValidationError) ? err.msg : err.to_s
    ...
    sdk_err = VatValidationError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(VatValidationError)
