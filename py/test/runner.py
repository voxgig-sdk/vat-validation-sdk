...
class VatValidationTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            VatValidationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = VatValidationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = VatValidationTestRunner.getenv("VAT_VALIDATION_TEST_LIVE")
        override = VatValidationTestRunner.getenv("VAT_VALIDATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = VatValidationTestRunner.getenv(key)
        ...
        explain = VatValidationTestRunner.getenv("VAT_VALIDATION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if VatValidationTestRunner._test_control is not None:
            return VatValidationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                VatValidationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            VatValidationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = VatValidationTestRunner.load_test_control()
...
def load_env_local():
    VatValidationTestRunner.load_env_local()


def env_override(m):
    return VatValidationTestRunner.env_override(m)


def entity_data(v):
    return VatValidationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return VatValidationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return VatValidationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return VatValidationTestRunner.load_test_control()


def live_client_options():
    return VatValidationTestRunner.live_client_options()


def live_delay_ms():
    return VatValidationTestRunner.live_delay_ms()
