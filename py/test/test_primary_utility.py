...
from vatvalidation_sdk.core.result import VatValidationResult
from vatvalidation_sdk.core.response import VatValidationResponse
from vatvalidation_sdk.core.operation import VatValidationOperation
from vatvalidation_sdk.core.error import VatValidationError
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return VatValidationError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = VatValidationResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = VatValidationSDK.test(None, None)
        ...
        class TestInitFeature(VatValidationBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(VatValidationBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = VatValidationSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = VatValidationSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = VatValidationSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = VatValidationOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = VatValidationSDK.test(None, setup_opts)
