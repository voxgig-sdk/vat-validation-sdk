...
module VatValidationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = VatValidationResponse.new({})
    ...
    if fetch_err
      ...
      response = VatValidationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = VatValidationResponse.new(fetched)
    else
