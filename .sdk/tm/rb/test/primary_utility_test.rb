...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = VatValidationOmni.make_runner(TEST_JSON_FILE, VatValidationSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    VatValidationError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : VatValidationOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = VatValidationOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
