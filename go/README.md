...
```go
validateIbanResponseSchema, err := client.ValidateIbanResponseSchema(nil).Load(map[string]any{"iban": "iban"}, nil)
if err != nil {
    panic(err)
}
...
```go
validateVatResponseSchema, err := client.ValidateVatResponseSchema(nil).Load(map[string]any{"vat_number": "vat_number"}, nil)
if err != nil {
    panic(err)
}
