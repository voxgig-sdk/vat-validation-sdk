...
import (
	...
	omni "github.com/voxgig-sdk/vat-validation-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.VatValidationError); is {
				out := map[string]any{
...
func omniCtx(arg any, client *sdk.VatValidationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.VatValidationSDK); is {
			client = live
