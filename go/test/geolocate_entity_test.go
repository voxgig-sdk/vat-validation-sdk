...
func TestGeolocateEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		geolocateRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.geolocate")))
...
func geolocateBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("VAT_VALIDATION_TEST_GEOLOCATE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["VAT_VALIDATION_TEST_GEOLOCATE_ENTID"])
