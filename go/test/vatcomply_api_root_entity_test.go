...
func TestVatcomplyApiRootEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set VATVALIDATION_TEST_VATCOMPLY_API_ROOT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func vatcomply_api_rootBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("VATVALIDATION_TEST_VATCOMPLY_API_ROOT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["VATVALIDATION_TEST_VATCOMPLY_API_ROOT_ENTID"])
