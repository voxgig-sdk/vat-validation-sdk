...
func TestCurrencyEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set VAT_VALIDATION_TEST_CURRENCY_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func currencyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("VAT_VALIDATION_TEST_CURRENCY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["VAT_VALIDATION_TEST_CURRENCY_ENTID"])
