...
func (sdk *VatValidationSDK) OptionsMap() map[string]any {
...
func (sdk *VatValidationSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *VatValidationSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *VatValidationSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *VatValidationSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *VatValidationSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *VatValidationSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("VatValidationSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *VatValidationSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *VatValidationSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("VatValidationSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *VatValidationSDK {
	...
	sdk := NewVatValidationSDK(sdkopts)
	sdk.Mode = "test"
