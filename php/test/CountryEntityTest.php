...
class CountryEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = VatValidationSDK::test($seed, null);
        $seen = iterator_to_array($base->Country(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = VatValidationConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = VatValidationSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set VAT_VALIDATION_TEST_COUNTRY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function country_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("VAT_VALIDATION_TEST_COUNTRY_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["VAT_VALIDATION_TEST_COUNTRY_ENTID"]);
