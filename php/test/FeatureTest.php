...
// VatValidation SDK feature test
//
...
class FtCtrl extends VatValidationControl
{
...
class FtClient
{
    ...
    private VatValidationUtility $utility;

    public function __construct(VatValidationUtility $utility, array $options)
    {
    ...
    public function get_utility(): VatValidationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (VatValidationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public VatValidationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (VatValidationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = VatValidationConfig::make_feature($name);
    ...
    private static function buildUrl(VatValidationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof VatValidationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new VatValidationResponse($fetched);
        }
    ...
    private function fail(VatValidationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (VatValidationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
