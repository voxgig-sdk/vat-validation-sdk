...
// VatValidation SDK retry feature
...
class VatValidationRetryFeature extends VatValidationBaseFeature
{
    ...
    public function init(VatValidationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (VatValidationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(VatValidationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(VatValidationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
