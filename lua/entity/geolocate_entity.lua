...
local GeolocateEntity = {}
GeolocateEntity.__index = GeolocateEntity


function GeolocateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GeolocateEntity)
  self._name = "geolocate"
  self._client = client
...
function GeolocateEntity:get_name()
  return self._name
end


function GeolocateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GeolocateEntity.new(self._client, opts)
end
...
function GeolocateEntity:mark_deleted()
  self._deleted = true
end


function GeolocateEntity:deleted()
  return true == self._deleted
end


function GeolocateEntity:data_set(args)
  if args ~= nil then
...
function GeolocateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GeolocateEntity:match_set(args)
  if args ~= nil then
...
function GeolocateEntity:match_get()
...
function GeolocateEntity:stream(action, args, callopts)
...
function GeolocateEntity:_run_op(ctx, post_done)
  local utility = self._utility
