// Command VatValidation-cli is a CLI + REPL for the VatValidation SDK,
...
const prompt = "vat-validation"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "country currency geolocate rate validate_iban_response_schema validate_vat_response_schema vatcomply_api_root"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      VAT_VALIDATION_APIKEY (api key), VAT_VALIDATION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: VAT_VALIDATION_APIKEY carries the API key and
	// VAT_VALIDATION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("VAT_VALIDATION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("VAT_VALIDATION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewVatValidationSDK(opts)
