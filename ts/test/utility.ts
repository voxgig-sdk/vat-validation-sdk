...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.VATVALIDATION_TEST_LIVE ||
    'TRUE' === process.env.VATVALIDATION_TEST_OVERRIDE
  ) {
  ...
  m.VATVALIDATION_TEST_EXPLAIN = process.env.VATVALIDATION_TEST_EXPLAIN || m.VATVALIDATION_TEST_EXPLAIN
