...
describe('ValidateVatResponseSchemaEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'validate_vat_response_schema.' + op, live)) return
    }
    ...
    const validate_vat_response_schema_ref01_data_dt0 = await validate_vat_response_schema_ref01_ent.load(validate_vat_response_schema_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['VAT_VALIDATION_TEST_VALIDATE_VAT_RESPONSE_SCHEMA_ENTID']
