...
describe('GeolocateDirect', async () => {
  ...
  test('direct-load-geolocate', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-geolocate', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VAT_VALIDATION_TEST_GEOLOCATE_ENTID']
