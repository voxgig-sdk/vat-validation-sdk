...
describe('VatcomplyApiRootDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new VatValidationSDK({
  ...
  test('direct-load-vatcomply_api_root', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-vatcomply_api_root', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VAT_VALIDATION_TEST_VATCOMPLY_API_ROOT_ENTID']
