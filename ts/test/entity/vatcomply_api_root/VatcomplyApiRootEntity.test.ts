...
describe('VatcomplyApiRootEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'vatcomply_api_root.' + op, live)) return
    }
    ...
    const vatcomply_api_root_ref01_data_dt0 = await vatcomply_api_root_ref01_ent.load(vatcomply_api_root_ref01_match_dt0)
