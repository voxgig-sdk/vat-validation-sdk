...
describe('ValidateIbanResponseSchemaDirect', async () => {
  ...
  test('direct-load-validate_iban_response_schema', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-validate_iban_response_schema', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VAT_VALIDATION_TEST_VALIDATE_IBAN_RESPONSE_SCHEMA_ENTID']
