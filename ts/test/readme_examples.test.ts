...
//     (\`new VatValidationSDK(...)\` / \`VatValidationSDK.test(...)\` become
//     \`VatValidationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'VatValidationSDK'
...
const TEST_SEED = {"entity":{"country":{"test01":{"id":"test01"}},"currency":{"test01":{"id":"test01"}},"geolocate":{"test01":{"id":"test01"}},"rate":{"test01":{"id":"test01"}},"validate_iban_response_schema":{"test01":{"id":"test01"}},"validate_vat_response_schema":{"test01":{"id":"test01"}},"vatcomply_api_root":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new VatValidationSDK(...)\` and any
// \`VatValidationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+VatValidationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/VatValidationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
