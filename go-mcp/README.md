...
[MCP](https://modelcontextprotocol.io) server exposing the VatValidation SDK as
two agent tools — `vat-validation_list` and `vat-validation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/vat-validation-mcp)
make build
...
claude mcp add --scope user vat-validation \
  -- /absolute/path/to/vat-validation-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./vat-validation-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// vat-validation_list: first page of records
{ "entity": "country" }
{ "entity": "country", "query": { } }

// vat-validation_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/vat-validation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user vat-validation \
     -- "$PWD"/dist/*/vat-validation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `vat-validation_list` and `vat-validation_load` tools now appear
   in new sessions. Ask the agent to *"list country using vat-validation"*
   and it calls `vat-validation_list` with `{"entity":"country"}`.
...
### Call the `vat-validation_list` tool
...
### Call the `vat-validation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `vat-validation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `vat-validation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./vat-validation-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"vat-validation_load","arguments":{"entity":"currency","query":{"id":1}}}}'
```
