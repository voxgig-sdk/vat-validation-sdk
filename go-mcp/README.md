...
MCP server exposing the VatValidation SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `vat-validation_list` | `entity`, optional `query` map | First page of records as JSON |
| `vat-validation_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `vat-validation_list` and
`vat-validation_load` tools become available in new sessions.
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"vat-validation_load","arguments":{"entity":"country","query":{"id":1}}}}'
```
